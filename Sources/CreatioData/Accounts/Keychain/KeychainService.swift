...
public class KeychainService {
    ...
    public static var standard = KeychainService()
    ...
    open func accessibilityOfKey(_ key: String) -> KeychainItemAccessibility? {
        ...
        return KeychainItemAccessibility(keychainItemAccessibilityKey: accessibilityAttrValue as CFString)
    }
    ...
    open func integer(forKey key: String, withAccessibility accessibility: KeychainItemAccessibility? = nil) -> Int? {
        guard let numberValue: NSNumber = object(forKey: key, withAccessibility: accessibility) else {
            return nil
        }
    ...
    open func object<T: NSObject & NSCoding>(forKey key: String, withAccessibility accessibility: KeychainItemAccessibility? = nil) -> T? {
        guard let keychainData = data(forKey: key, withAccessibility: accessibility) else {
            return nil
        }
    ...
    @discardableResult open func set(_ value: Data, forKey key: String, withAccessibility accessibility: KeychainItemAccessibility? = nil) -> Bool {
        ...
        if let accessibility = accessibility {
            keychainQueryDictionary[SecAttrAccessible] = accessibility.keychainAccessibilityAttributeKey
        } else {
            // Assign default protection - Protect the keychain entry so it's only valid after the first device unlock
            keychainQueryDictionary[SecAttrAccessible] = KeychainItemAccessibility.afterFirstUnlock.keychainAccessibilityAttributeKey
        }
